...
export class PermissionService {
  ...
  getPermissions() {
    ...
    return this.http.get(permissionListUrl).toPromise().then(response => response.json() as Permission[]).catch(this.handleError);
  }

  getPermissionById(id: number) {
    let permissionUrl = this.config.Server + 'nsdc/v1.0/permissions/' + id;
    return this.http.get(permissionUrl).toPromise().then(response => response.json() as Permission).catch(this.handleError);
  }
